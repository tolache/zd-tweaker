...
const observer = new MutationObserver(() => {
    ...
    if (infoBars.length > 0) {
        infoBars.forEach((infoBar) => {
            hideSharedTicketInfoBar(infoBar);
            hideArchivedTicketInfoBar(infoBar);
            hideFollowUpInfoBars(infoBar);
        });
    }
});
...
function hideSharedTicketInfoBar(infoBar: HTMLDivElement) {
    if (infoBar.textContent?.trim().startsWith("Shared with")) {
        hideInfoBarParentNode(infoBar);
    }
}

function hideArchivedTicketInfoBar(infoBar: HTMLDivElement) {
    if (infoBar.textContent?.trim().startsWith("This is an archived ticket")) {
        hideInfoBarParentNode(infoBar);
    }
}

function hideFollowUpInfoBars(infoBar: HTMLDivElement) {
...
function hideInfoBarParentNode(infoBar: HTMLDivElement) {
